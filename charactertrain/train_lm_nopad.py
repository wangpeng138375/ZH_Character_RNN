...
import charactertrain.reader_nopad as reader
...
def main(_):
    ...
    all_data, id_to_word, vocabulary_length = reader.ptb_raw_data(config)
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Train"):
            train_input = inputLM(config=config, data=train_data, name="TrainInput")
        ...
        with tf.name_scope("Valid"):
            valid_input = inputLM(config=config, data=valid_data, name="ValidInput")
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                mvalid = LM(is_training=False, config=config, input_=valid_input)
            tf.scalar_summary("Validation Loss", mvalid.cost)
    
        with tf.name_scope("Test"):
          test_input = inputLM(config=eval_config, data=test_data, name="TestInput")
          with tf.variable_scope("Model", reuse=True, initializer=initializer):
            mtest = LM(is_training=False, config=eval_config,
                             input_=test_input)
        ...
        with sv.managed_session() as session:
            ...
            if config["save_path"]:
              print('saving final model to {0}.final'.format(config['name']))
